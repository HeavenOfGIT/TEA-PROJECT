				...
				struct smb1351_charger *chip)
{
	...
	if (rc)
		pr_debug("Couldn't set charging current rc = %d\n", rc);
}
	...
	*/
	if (chip->bms_psy) {
		rc = power_supply_get_property(chip->bms_psy,
				POWER_SUPPLY_PROP_CAPACITY, &ret);
		if (rc) {
			pr_debug("Couldn't read the bms capacity rc = %d\n",
									rc);
			return;
		}
		if (!chip->batt_cool && !chip->batt_warm
...
static void smb1351_chg_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	if (state >= ADC_TM_STATE_NUM) {
		pr_debug("invalid state parameter %d\n", state);
		return;
	}
...
static void smb1351_external_power_changed(struct power_supply *psy)
{
	...
	if (rc)
		pr_debug("Couldn't read USB online property, rc=%d\n", rc);
	else
		online = prop.intval;
...
static int smb1351_determine_initial_state(struct smb1351_charger *chip)
{
	...
	rc = smb1351_read_reg(chip, IRQ_G_REG, &reg);
	if (rc) {
		pr_debug("Couldn't read IRQ_G rc = %d\n", rc);
		goto fail_init_status;
	}
				...
				const struct i2c_device_id *id)
{
	...
	if (rc) {
		pr_debug("Couldn't intialize hardware rc=%d\n", rc);
